        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    jobOpenings.map((job) => (
                        <div key={job.id} className="bg-white dark:bg-navy-800 p-6 rounded-lg shadow-sm border border-gray-100 dark:border-navy-700 hover:border-gold-500 transition-colors">
                            <div className="flex justify-between items-start flex-wrap gap-4">
                                <div>
                                    <h3 className="text-xl font-bold text-navy-900 dark:text-white">{job.title}</h3>
                                    <div className="flex gap-3 mt-2 text-sm text-gray-500 dark:text-gray-400">
                                        <span className="bg-gray-100 dark:bg-navy-900 px-2 py-1 rounded">{job.type}</span>
                                        <span className="bg-gray-100 dark:bg-navy-900 px-2 py-1 rounded">{job.location}</span>
                                    </div>
                                    <p className="mt-4 text-gray-600 dark:text-gray-300">{job.desc}</p>
                                </div>
            ...
            <div className="lg:col-span-1">
                <div className="bg-white dark:bg-navy-800 p-8 rounded-lg shadow-lg sticky top-24">
                    <h3 className="text-xl font-bold text-navy-900 dark:text-white mb-4">General Application</h3>
                    <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">
                        Don't see a fitting role? Send us your CV and we'll contact you when a position opens.
                    </p>
                    <form className="space-y-4">
                        ...
                        <input type="email" placeholder="Email" className="w-full px-4 py-2 rounded border border-gray-300 dark:border-navy-600 bg-transparent dark:text-white" />
                        <textarea placeholder="Cover Letter" rows={3} className="w-full px-4 py-2 rounded border border-gray-300 dark:border-navy-600 bg-transparent dark:text-white"></textarea>
